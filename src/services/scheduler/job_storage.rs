          ...
          Tz::Offset: Send + Sync
{
    ...
    async fn add_job(&self, job_name: String, cron: String, args: Option<Value>) -> Result<String, SchedulerError> {
        ...
        let cron_scheduler = cron::Schedule::from_str(&cron).map_err(|_| SchedulerError::new(SchedulerErrorKind::CronInvalid))?;
        ...
        entity::job_storage::ActiveModel {
            ...
            args: Set(args),
        }.insert(Database::get_conn()).await.map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("数据库存储错误".into())))?;
        Ok(id.to_string())
    }

    async fn delete_job(&self, id: String) -> Result<(), SchedulerError> {
        entity::job_storage::Entity::delete_by_id(Uuid::from_str(&id).map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("Id不合法".into())))?)
            .exec(Database::get_conn()).await.map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("数据库删除失败".into())))?;

        Ok(())
    }

    async fn has_job(&self, id: String) -> Result<bool, SchedulerError> {
        let count = entity::job_storage::Entity::find_by_id(Uuid::from_str(&id).map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("Id不合法".into())))?)
            .count(Database::get_conn()).await.map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("数据库读取失败".into())))?;
    ...
    async fn get_all_should_execute_jobs(&self) -> Result<Vec<Pin<Box<dyn Future<Output=()> + Send>>>, SchedulerError> {
        ...
        let tasks = entity::job_storage::Entity::find()
            ...
            .await.map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("数据库读取失败".into())))?;
        ...
        for task in tasks {
            ...
            let last_execute_at = match chrono::NaiveDateTime::from_timestamp_opt(task.last_check_at, 0) {
                Some(e) => e,
                None => continue
            };

            let last_execute_at = chrono::DateTime::<chrono::Utc>::from_utc(last_execute_at, chrono::Utc).with_timezone(&self.timezone);
            ...
            entity::job_storage::Entity::update_many()
                ...
                .col_expr(entity::job_storage::Column::NextExecuteAt, Expr::value(next_execute_at))
                .col_expr(entity::job_storage::Column::LastCheckAt, Expr::value(time_now.timestamp()))
                .exec(Database::get_conn())
                .await
                .map_err(|_| SchedulerError::new(SchedulerErrorKind::CustomErr("数据库更新失败".into())))?;
        }
