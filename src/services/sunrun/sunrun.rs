...
impl Sunrun {
    ...
    pub async fn get_token(&mut self, is_iphone: bool) -> BusinessResult<String> {
        let path = if is_iphone { "/%7Btoken%7D/QM_Users/LoginSchool" } else { "/%7Btoken%7D/QM_Users/Login_AndroidSchool" };
    ...
    pub async fn get_userinfo(&self) -> BusinessResult<SunrunUserInfo> {
        ...
        Ok(SunrunUserInfo {
            length: school_run["Lengths"].as_i64().ok_or(BusinessError::new("跑步长度为空"))?,
            max_speed: school_run["MaxSpeed"].as_f64().ok_or(BusinessError::new("跑步最大速度为空"))?,
            min_speed: school_run["MinSpeed"].as_f64().ok_or(BusinessError::new("跑步最小速度为空"))?,
            school_name: school_run["SchoolName"].as_str().unwrap_or("").to_owned(),
            nick_name: user["NickName"].as_str().unwrap_or("").to_owned(),
            user_id: user["UserID"].as_i64().ok_or(BusinessError::new("用户ID为空"))?,
            latitude: String::new(),
    ...
    pub async fn get_run_times(&self) -> BusinessResult<SunrunRunTimes> {
        ...
        let result = HttpClient::client()
            .get(format!("{BASE_URL}/{token}/QM_Runs/getResultsofValidByUser"))
            .query(&[("UserId", user_info.user_id), ("pageIndex", 1), ("pageSize", u8::MAX as i64)])
            .send()
            .await?
            .ensure_success_json()
            .await?;
        ...
        Ok(SunrunRunTimes {
            morning_run_times: result["RaceMNums"].as_i64().ok_or(BusinessError::new("晨跑次数不存在"))?,
            run_times: result["RaceNums"].as_i64().ok_or(BusinessError::new("总跑步次数不存在"))?,
        })
    }
    ...
    pub async fn start_run(&self, userinfo: SunrunUserInfo) -> BusinessResult<()> {
        ...
        let run_info = HttpClient::client()
            ...
            .query(&[("S1", &userinfo.latitude), ("S2", &userinfo.longitude), ("S3", &userinfo.length.to_string())])
            .set_version_header()
            .send()
            .await?
            .ensure_success_json()
        ...
        let run_id = run_info["Data"]["RunId"].as_str().ok_or(BusinessError::new("RunId为空"))?;
        let route = run_info["Data"]["Routes"].as_str().ok_or(BusinessError::new("Routes为空"))?;
        ...
        let _ = HttpClient::client().get(format!("{BASE_URL}/{token}/QM_Runs/ES"))
            .query(&[("S1", run_id), ("S4", &SunrunCrypto::encrypt_integer(&encrypt_key, run_time))
                , ("S5", &SunrunCrypto::encrypt_integer(&encrypt_key, run_length)), ("S6", route), ("S7", "1")
                , ("S8", &encrypt_key_str), ("S9", &SunrunCrypto::encrypt_integer(&encrypt_key, step))])
            .send()
            .await?
            .ensure_success_json()
            .await?;
...
impl SunrunHttpResponseExt for reqwest::Response {
    async fn ensure_success_json(self) -> BusinessResult<serde_json::Value> {
        ...
        if !json["Success"].as_bool().unwrap_or(false) {
            return Err(BusinessError::new(json["ErrMsg"].as_str().unwrap_or("阳光体育服务器请求失败")));
        }
