...
impl SunrunTaskManager {
    pub async fn create_task(user_info_id: i64, hour: i32, minute: i32) -> BusinessResult<i64> {
        let id = Scheduler::get()
            .add_job(
                crate::tasks::sunrun_task::TASK_NAME.to_owned(),
                format!("0 {minute} {hour} * * * *"),
                Some(json!({ "id": user_info_id })),
            )
            .await?;
    ...
    pub async fn update_task_enable(user_info_id: i64, is_enable: bool) -> BusinessResult<()> {
        ...
        if is_enable {
            if !Scheduler::get()
                .has_job(task_info.task_id.to_string())
                .await?
            {
                ...
                Scheduler::get()
                    .add_job(
                        crate::tasks::sunrun_task::TASK_NAME.to_owned(),
                        format!("0 {} {} * * * *", task_info.minute, task_info.hour),
                        Some(json!({ "id": user_info_id })),
                    )
                    .await?;
            }
            ...
            Scheduler::get()
                .delete_job(task_info.task_id.to_string())
                .await?;
        }
